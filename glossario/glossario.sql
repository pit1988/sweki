...
('best_practice', 'best practice', 'la prassi che, per esperienza e per studio, abbia mostrato di garantire i migliori risultati in circostanze note e specifiche', 'best practice'),
('prassi', 'prassi', 'modo di fare', 'practice'),
('stakeholder', 'stakeholder', 'persona a vario titolo coinvolta nel ciclo di vita di un software che ha influenza sul prodotto o sul processo', 'stakeholder'),
...
('ciclo_PDCA', 'ciclo PDCA (o ciclo di Deming)', 'schema iterativo di auto-miglioramento che consiste di quattro punti: Plan (individuare obiettivi di miglioramento), Do (eseguire ciò che si è pianificato), Check (verificare se ha funzionato) e Act (agire per correggersi)', 'PDCA cycle'),
...
('slack_time', 'slack time', 'quantità di tempo tra la data minima a partire da cui un evento può accadere e la data massima oltre la quale esso ritarda gli eventi successivi', 'slack time'),
...
('budget', 'budget', 'tempo e denaro', 'budget'),
('UML', 'UML (Unified Modelling Language)', 'famiglia di notazioni grafiche che si basano su un singolo meta-modello e servono a supportare la descrizione e il progetto dei sistemi software', 'UML'),
...
('diagramma_dei_casi_d_uso', 'diagramma dei casi d\'uso', 'grafo orientato che mostra gli attori, i casi d\'uso e le relazioni tra essi: ogni nodo è un attore o un caso d\'uso; ogni arco è una comunicazione tra un attore e un caso d\'uso oppure una relazione (di estensione, inclusione o generalizzazione) tra due casi d\'uso o tra due attori', 'use case diagram'),
...
('baseline', 'baseline', 'nel ciclo di vita di un progetto, punto d\'arrivo tecnico dal quale non si retrocede', 'baseline'),
('milestone', 'milestone', 'punto nel tempo associato ad un valore strategico', 'milestone'),
('repository', 'repository', 'base di dati centralizzata nella quale risiedono, individualmente, tutti i CI di ogni baseline nella loro storia completa', 'repository'),
...
('regola', 'regola', 'norma di progetto sottoposta a verifica', 'rule'),
...
('componente', 'componente', 'parte di un sistema. Un insieme di unità funzionalmente coese; in Java, rappresentabile da uno o più package.', 'component'),
('unità', 'unità', 'un insieme coeso di moduli, appaltabili in realizzazione a un singolo programmatore; non ha sempre un corrispondente diretto in un linguaggio di programmazione. La più piccola quanità di software che conviene verificare da sola', 'unit'),
('modulo', 'modulo', 'l\'elemento atomico della progettazione; tipicamente una classe o un interfaccia.', 'module'),
('requisito', 'requisito', 'bisogno da soddisfare o vincolo da rispettare', 'requirement'),
('requisito_utente', 'requisito utente', 'richiesta generale, ad alto livello', 'user requirement'),
('requisito_di_sistema', 'requisito di sistema', 'definizione formale e dettagliata di una funzione del sistema', 'system requirement'),
...
('requisito_funzionale', 'requisito funzionale', '(di un prodotto software) servizio che il prodotto deve fornire', 'functional requirement'),
('requisito_non_funzionale', 'requisito non funzionale', '(di un prodotto software) vincolo su uno o più servizi che il prodotto fornisce', 'non-functional requirement'),
('architettura', 'architettura', 'la struttura organizzativa di un sistema o componente', 'architecture'),
('coesione', 'coesione', 'caratteristica di un sistema per la quale parti associate concorrono agli stessi obiettivi', 'cohesion'),
...
('misura', 'misura', 'risultato di una misurazione', NULL);
